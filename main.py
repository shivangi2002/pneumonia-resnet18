...
def run_model(lr, images_per_batch, num_epochs, augment=False, patience = 5, class_weights = None, fine_tune = False):
    torch.manual_seed(42)
    aug_suffix = "_aug" if augment else ""
    cw_suffix = f"_w{int(class_weights[0])}" if class_weights and class_weights[0] != 1.0 else ""
    ft_suffix = "_ft" if fine_tune else ""
    ...
    save_run_summary(hyperparameters, augment, class_weights, fine_tune, best_epoch, history, csv_path)
